  ...
  return (
    <div className="p-6 lg:p-10">
      ...
      <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <h1 className="text-xl font-semibold text-[#171717] min-w-0 break-words">{data.title}</h1>
        <span className="w-fit shrink-0 rounded-full bg-[#F0F0F0] px-2.5 py-0.5 text-xs font-medium text-[#525252]">{data.status.replace(/_/g, " ")}</span>
      </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-[#E5E5E5] bg-white p-6">
          ...
          <div className="mt-4 space-y-3">
            <div className="flex flex-col sm:flex-row sm:justify-between gap-0.5 sm:gap-2 border-b border-[#F0F0F0] pb-2"><span className="text-sm text-[#737373]">Category</span><span className="text-sm text-[#171717]">{data.category}</span></div>
            <div className="flex flex-col sm:flex-row sm:justify-between gap-0.5 sm:gap-2 border-b border-[#F0F0F0] pb-2"><span className="text-sm text-[#737373]">Amount</span><span className="text-sm text-[#171717]">{data.amountRequested ? `₦${data.amountRequested.toLocaleString()}` : "Not specified"}</span></div>
            <div className="flex flex-col sm:flex-row sm:justify-between gap-0.5 sm:gap-2"><span className="text-sm text-[#737373]">Submitted</span><span className="text-sm text-[#171717]">{new Date(data.createdAt).toLocaleDateString()}</span></div>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-xl border border-[#E5E5E5] bg-white p-6">
          ...
          <div className="mt-4 space-y-3">
            {data.disbursements.map((d) => (
              <div key={d._id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2 rounded-lg border border-[#F0F0F0] px-3 py-2">
                ...
                <span className={`w-fit shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium ${d.evidenceStatus === "verified" ? "bg-[#E6FBF0] text-[#00D632]" : d.evidenceStatus === "overdue" ? "bg-red-50 text-red-600" : "bg-[#F0F0F0] text-[#737373]"}`}>
