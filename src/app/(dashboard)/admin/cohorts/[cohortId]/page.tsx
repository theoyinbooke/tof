...
function OverviewTab({
  ...
  const pillarEntries = Object.entries(PILLAR_CONFIG).map(([pillar, config]) => ({
    pillar,
    label: config.label,
    color: config.color,
    count: cohortStats.pillarCounts[pillar] ?? 0,
  }));
  const hasPillarData = pillarEntries.some(({ count }) => count > 0);
  const maxPillarCount = Math.max(...pillarEntries.map(({ count }) => count), 1);
  ...
  return (
    <div className="space-y-6">
      ...
      {hasPillarData && (
        <div className="rounded-xl border border-[#E5E5E5] bg-white p-6">
          <h3 className="mb-6 text-sm font-semibold text-[#171717]">
            Pillar Distribution
          </h3>
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
            {pillarEntries.map(({ pillar, label, color, count }) => {
              const percentage = Math.round((count / maxPillarCount) * 100);
              return (
                <div
                  key={pillar}
                  className="rounded-xl border border-[#F0F0F0] bg-[#FCFCFC] p-4"
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="rounded-full bg-white px-2 py-1 text-[11px] font-medium text-[#737373]">
                      {percentage}%
                    </span>
                  </div>

                  <div className="mt-4">
                    <div className="relative flex h-44 items-end overflow-hidden rounded-lg bg-[#F5F5F5] px-6 py-3">
                      <div className="pointer-events-none absolute inset-x-0 bottom-3 border-t border-dashed border-[#E5E5E5]" />
                      <div className="pointer-events-none absolute inset-x-0 top-1/3 border-t border-dashed border-[#EEEEEE]" />
                      <div className="pointer-events-none absolute inset-x-0 top-[18%] border-t border-dashed border-[#F3F3F3]" />
                      <div
                        className="relative z-10 mx-auto w-full max-w-[72px] rounded-t-xl transition-all duration-300"
                        style={{
                          height: `${Math.max(percentage, count > 0 ? 12 : 0)}%`,
